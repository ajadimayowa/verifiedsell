...
const CustomInput: React.FC<any> = ({ type }) => {
    ...
    const handleSearch = ()=>{
        if(searchWord==''){
            ...
            const searched = results.filter((res)=>(res.title.toLocaleLowerCase().includes(searchWord.toLocaleLowerCase())))
    ...
    if (type == 'text') {
        ...
        return (
            ...
            <form className="d-flex flex-row gap-2  bg-light align-items-center border rounded rounded-1 p-1" style={{width:'25em'}}>

                <input 
                placeholder="Search any item..."
                onChange={(e)=>setSearchWord(e.target.value)}
                className="d-flex border border-0 flex-row gap-2 bg-light p-2" style={{outline:'none',width:'75%'}}/>
                ...
                <i className="bi bi-search p-2 rounded bg-secondary px-3" role="button"></i>
                </div>
            </form>
            {
            searchWord.length>0&&
            <div className="bg-light p-1 text-dark w-100" style={{position:'absolute',top:55}}>
            <ul className="d-flex align-items-start gap-2 flex-column m-0 p-0">
                {
                ...
                }
                    {
                        searchResult.map((link, index) => (
                            <li key={index} className="list-group-item w-100 p-2 fw-medium d-flex gap-2" role="button">
                                <i className={link.icon}></i>{link.title} <p className="p-0 m-0">{link.path}</p>
                            </li>
                        ))
